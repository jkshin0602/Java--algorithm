...
public class GraphConception {
	/*그래프(Graph)란?
	그래프는 실제 세계의 현상이나 사물을 정점(Vertex)또는 노드(Node)와 간전(Edge)로 표현하기 위해 사용
	...
	간선에 방향이 있는 그래프
	보통 노드 A, B가 A->B로 가는 간선으로 연결되어 있을 경우, <A, B>로 표기(<B,A>는 B->A로 가는 간선이 있는 경우이므로
		<A,B>와 다름)
	...
	연결 그래프(Connected Graph)
		무 방향 그래프에 있는 모든 노드에 대해 항상 경로가 존재하는 경우
	비연결 그래프(Diconnectd Graph)
